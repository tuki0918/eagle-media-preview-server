...
function createEagleClient({ baseUrl = process.env.EAGLE_BASE_URL || "http://localhost:41595", token = process.env.EAGLE_TOKEN || "", fetchImpl = globalThis.fetch, } = {}) {
    ...
    return {
        ...
        async switchLibrary(libraryPath) {
            return unwrapData(await request("/api/library/switch", {
                method: "POST",
                body: { libraryPath },
            }));
        },
        async folders({ offset = 0, limit = 1000 } = {}) {
            return normalizePaginatedResponse(await request("/api/v2/folder/get", {
        ...
        async listItems({ offset = 0, limit = 30, folderId, isUnfiled = false, ext, rating, keywords, tags } = {}) {
            ...
            if (cleanTags.length)
                body.tags = cleanTags;
            return normalizePaginatedResponse(await request("/api/v2/item/get", { method: "POST", body }));
        },
        async searchItems({ query, offset = 0, limit = 30 }) {
            return normalizePaginatedResponse(await request("/api/v2/item/query", {
                method: "POST",
                body: { query, offset: normalizeOffset(offset), limit: clampLimit(limit) },
            }));
        },
        async listTags({ query = "", offset = 0, limit = 20 } = {}) {
            return normalizePaginatedResponse(await request("/api/v2/tag/get", {
                searchParams: {
        ...
        async itemById(id) {
            return unwrapData(await request("/api/v2/item/get", {
                method: "POST",
        ...
        async updateItemStar(id, star) {
            ...
            return unwrapData(await request("/api/v2/item/update", {
                method: "POST",
        ...
        async updateItemMetadata(id, { tags, folders } = {}) {
            const body = { id };
            if (tags !== undefined)
                body.tags = normalizeStringArray(tags, "tags");
            if (folders !== undefined)
                body.folders = normalizeStringArray(folders, "folders");
            if (!body.id)
