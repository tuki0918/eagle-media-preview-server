...
function createViewerServer({ host = "0.0.0.0", port = 41532, publicDir = defaultPublicDir, viewerPassword = "", passwordHash = "", basicAuthUsername = "eagle", eagleClient = createEagleClient(), } = {}) {
    ...
    const server = createServer(async (req, res) => {
        try {
            ...
            if (url.pathname.startsWith("/api/")) {
                ...
                if (await handleAuthRoutes(req, url, res, auth))
                    return;
...
async function handleApi(req, url, res, { getSession, setSession }) {
    ...
    if (url.pathname === "/api/items") {
        ...
        const tags = [...url.searchParams.getAll("tags"), ...url.searchParams.getAll("tag")]
            .map((tag) => tag.trim())
            .filter(Boolean);
        const hasStructuredFilters = Boolean(url.searchParams.get("folderId") || url.searchParams.get("ext") || url.searchParams.get("rating") || tags.length);
...
function mediaFileName(item, filePath) {
    ...
    if (!ext || fallbackName.toLowerCase().endsWith(`.${ext.toLowerCase()}`))
        return fallbackName;
...
function isAuthorized(req, auth) {
    if (!authRequired(auth))
        return true;
    if (basicAuthMatches(req.headers.authorization || "", auth))
        return true;
