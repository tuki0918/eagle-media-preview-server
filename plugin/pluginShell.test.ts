...
test("plugin window uses per-user roles for metadata permissions", async () => {
  ...
  assert.match(app, /type=\{passwordVisibleByIndex\[String\(index\)\] \? "text" : "password"\}/);
  assert.match(app, /aria-label=\{passwordVisibleByIndex\[String\(index\)\] \? `Hide password for \$\{user\.username \|\| `user \$\{index \+ 1\}`\}` : `Show password for \$\{user\.username \|\| `user \$\{index \+ 1\}`\}`\}/);
  assert.match(app, /onClick=\{\(\) => togglePasswordVisible\(index\)\}/);
  assert.match(app, /passwordVisibleByIndex\[String\(index\)\] \? <EyeIcon className="h-\[13px\] w-\[13px\]" \/> : <EyeOffIcon className="h-\[13px\] w-\[13px\]" \/>/);
