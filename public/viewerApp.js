const state = {
  ...
  previewDrag: null,
  previewPointers: /* @__PURE__ */ new Map(),
  previewPinch: null,
  previewItemId: "",
...
const playableVideoExts = /* @__PURE__ */ new Set(["mp4", "webm", "mov", "m4v"]);
const playableAudioExts = /* @__PURE__ */ new Set(["mp3", "wav", "m4a", "aac", "ogg"]);
const textPreviewExts = /* @__PURE__ */ new Set([
  "txt",
  "md",
  "js",
  "css",
...
const pdfPreviewExts = /* @__PURE__ */ new Set(["pdf"]);
const videoExts = /* @__PURE__ */ new Set([...playableVideoExts, "avi", "mkv"]);
const audioExts = /* @__PURE__ */ new Set([...playableAudioExts, "flac", "wma"]);
...
function libraryLabel(data) {
  var _a, _b;
  const name = ((_a = data.library) == null ? void 0 : _a.name) || LIBRARY_EMPTY_LABEL;
  const version = ((_b = data.app) == null ? void 0 : _b.version) ? `Eagle ${data.app.version}` : EAGLE_UNAVAILABLE_LABEL;
  return `${name} - ${version}`;
}
...
function bindPreviewTrigger(element, item) {
  ...
  const openFromPointer = (event) => {
    ...
    if ((event == null ? void 0 : event.type) === "touchend") {
      event.preventDefault();
    }
...
function populateThumb({ img, badge, duration, item }) {
  ...
  button == null ? void 0 : button.classList.add("thumb-loading");
  img.loading = "lazy";
  img.decoding = "async";
  img.src = mediaUrl(item.id, "thumb");
  img.alt = item.name || item.id;
  img.onload = () => {
    button == null ? void 0 : button.classList.remove("thumb-loading");
    img.hidden = false;
  };
  img.onerror = () => {
    button == null ? void 0 : button.classList.remove("thumb-loading");
    img.hidden = true;
    button == null ? void 0 : button.classList.add("thumb-missing");
  };
  if (badge) {
...
function decorateThumbButton(button, overlayIcon, item) {
  ...
  const mediaType = playableVideoExts.has(ext) ? "video" : playableAudioExts.has(ext) ? "audio" : textPreviewExts.has(ext) || pdfPreviewExts.has(ext) ? "document" : "image";
...
function resetPreviewTransform() {
  ...
  state.previewPointers = /* @__PURE__ */ new Map();
...
function endImageDrag(event) {
  if ((event == null ? void 0 : event.pointerType) === "touch") {
...
function videoErrorMessage(error) {
  if ((error == null ? void 0 : error.code) === MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED) {
...
async function loadTagSuggestions() {
  ...
  try {
    ...
    renderTagSuggestions(items.filter((item) => (item == null ? void 0 : item.name) && !state.tags.includes(item.name)));
  } catch {
...
function tagSuggestionItems(query, selectedValues) {
  ...
  const recent = readRecentList(RECENT_TAGS_STORAGE_KEY).filter((tag) => !selected.has(tag) && matchesQuery(tag, query)).map((tag) => ({ value: tag, label: tag, meta: "Recent" }));
  if (!query) return recent;
  const params = new URLSearchParams({ q: query, limit: "20" });
  return getJson(`/api/tags?${params.toString()}`).then((data) => {
    const remote = Array.isArray(data.items) ? data.items : [];
    return dedupeSuggestions([
      ...recent,
      ...remote.map((item) => ({
        value: normalizeTag(item == null ? void 0 : item.name),
        label: normalizeTag(item == null ? void 0 : item.name),
        meta: Number.isFinite(item == null ? void 0 : item.count) ? item.count.toLocaleString() : ""
...
function folderSuggestionItems(query, selectedValues) {
  const selected = new Set(selectedValues);
  const recent = readRecentList(RECENT_FOLDERS_STORAGE_KEY).filter((id) => !selected.has(id) && matchesQuery(folderLabel(id), query)).map((id) => ({ value: id, label: folderLabel(id), meta: "Recent" }));
  const folders = state.folders.filter((folder) => !selected.has(folder.id) && matchesQuery(folder.name, query)).map((folder) => ({
    value: folder.id,
...
function formatDate(value) {
  const timestamp = typeof value === "string" && value.trim() && !Number.isFinite(Number(value)) ? Date.parse(value) : Number(value);
...
function emptyStateNode() {
  ...
  description.textContent = hasActiveFilters() ? "Try changing the search text, folder, extension, or rating to widen the results." : "This page has no items yet. Refresh or change the current view to load another range.";
