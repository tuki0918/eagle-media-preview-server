...
- `plugin/service/*.cts`
  ...
  - Compiled by `tsconfig.plugin-service.json` to CommonJS `.cjs` files for Eagle's `require` runtime
...
`npm run build` compiles the plugin service `.cts` files before Vite builds and packages the React surfaces. The Vite package step also copies `manifest.json`, plugin icons/assets, public assets, favicon, generated service `.cjs` files, and CSS into `dist`.
...
- Generated `plugin/service/*.cjs` files are build artifacts and are not tracked as source.
